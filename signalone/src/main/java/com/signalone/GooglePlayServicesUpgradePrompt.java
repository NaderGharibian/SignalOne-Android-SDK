...
class GooglePlayServicesUpgradePrompt {
   ...
   static boolean isGMSInstalledAndEnabled() {
      try {
         PackageManager pm = SignalOne.appContext.getPackageManager();
   ...
   static void ShowUpdateGPSDialog() {
      ...
      boolean userSelectedSkip =
         SignalOnePrefs.getBool(
            SignalOnePrefs.PREFS_ONESIGNAL,
            SignalOnePrefs.PREFS_GT_DO_NOT_SHOW_MISSING_GPS,
            false
         );
      if (userSelectedSkip)
         return;
      ...
      OSUtils.runOnMainUIThread(new Runnable() {
         ...
         public void run() {
            ...
            if (activity == null || SignalOne.mInitBuilder.mDisableGmsMissingPrompt)
               return;
            ...
            builder.setMessage(alertBodyText).setPositiveButton(alertButtonUpdate, new DialogInterface.OnClickListener() {
               ...
               public void onClick(DialogInterface dialog, int which) {
                  SignalOnePrefs.saveBool(SignalOnePrefs.PREFS_ONESIGNAL,
                     SignalOnePrefs.PREFS_GT_DO_NOT_SHOW_MISSING_GPS,true);
   ...
   private static void OpenPlayStoreToApp(Activity activity) {
      try {
         ...
         int resultCode = apiAvailability.isGooglePlayServicesAvailable(SignalOne.appContext);
